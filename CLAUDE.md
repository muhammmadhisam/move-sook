...
pnpm typecheck                   # tsc --noEmit across all 10 workspaces (3 apps + 7 packages) — the primary gate
...
Three apps (`api`, `web`, `admin`) over seven shared packages (`config`, `db`, `shared`, `auth`, `services`, `ui`, `thailand-provinces`). Dependency flow: `config` ← `db` ← `shared` ← `auth` ← `services` ← `api` → consumed by `web`/`admin` (which also use `ui`).

**`@movesook/services` is the API's domain + infrastructure core.** Route handlers in `apps/api/src/routes/` are **thin HTTP wrappers** (middleware + `zValidator` + `c.json`) that call function+module services in `packages/services/src/<context>/` (`jobs`, `drivers`, `admin`, `auth`, `me`, `webhooks`, `blog`) — services take the authenticated `sub` + validated input and return wire DTOs (or throw Hono `HTTPException`). HTTP-only concerns stay in the route: cookie writes (`auth`), `streamSSE` (`jobs` tracking), PDF/CSV response headers, and the `x-line-signature` raw-body read (`webhooks`); services never receive the Hono `c`. The domain-support + async layer moved out of `apps/api/src/lib` and `apps/api/src/queues` into `packages/services/src/support/` (re-exported from `@movesook/services/support`). Env validation still fails fast in `apps/api/src/config.ts`, which hands the result to `configureServices(env)` once at boot; package modules read it lazily via `getEnv()` from `@movesook/services/runtime` (never at import time). What stays in `apps/api/src/lib`: `context.ts` (Hono `AppEnv`) and `cookies.ts` (HTTP cookie writer); `uploads.ts` keeps its R2/disk storage logic per its own boundary. `thailand-provinces` is a leaf **data package** (Thai provinces/amphures/tambons/zip codes + address helpers) consumed by `shared` (province validation), `ui`, `web`, and `admin` — it exports source JSON + helpers, never the Prisma client.
...
**API surface.** Routes live in `apps/api/src/routes/` and mount into the `app.ts` chain: `auth` (LINE + admin login), `me` (USER/DRIVER session, profile, in-app notifications), `jobs` (post / accept / `:id/status` / `:id/proof` / `:id/cancel` / `:id/review` + the driver feed), `drivers` (`/claim` to apply, `/me` profile, `/me/availability`, `/me/earnings`), `uploads`, `webhooks`, and a broad `admin` surface (stats/analytics, users & customers, jobs, transactions, payouts, disputes, settings — commission/pricing/system, service-areas, vehicle-pricing, promos, blacklist, audit-logs, admin invites, and PDPA consents/export/anonymize). Driver *verification* is an admin action (`admin/drivers/:id`), not a driver route. Cross-cutting helpers live in `@movesook/services/support` (`packages/services/src/support/`): `transactions` (commission ledger), `settings` (cached `AppSetting` reads), `notify` (in-app notifications + LINE-push enqueue), `audit` (`AuditLog` writes for admin actions), `rate-limit` (Redis-backed admin-login lockout), `redis` (shared ioredis connections — exported as the lazy getters `redis()`/`bullConnection()`), `paginate`, and `serialize`; `cookies` and `context` (Hono `AppEnv`/claims typing) stay in `apps/api/src/lib/`. **Redis + BullMQ** back the async layer: `packages/services/src/support/queues/` holds three queues + their workers, started from the API's `index.ts` via `startWorkers()` (re-exported from `@movesook/services/support`) (gated by `WORKERS_ENABLED`; `REDIS_URL` required; `docker compose up -d redis` for local dev): `notifications` (LINE push), `maintenance` (repeatable cron jobs), and `side-effects` (durable post-commit writes that were previously best-effort try/catch — `writeAudit()` now enqueues the `AuditLog` row, and `maybeIssueReferralReward()` enqueues the referral grant). The referral grant (`runReferralRewardGrant` in `support/referral.ts`) claims `referralRewardedAt` and creates both reward promos in **one transaction** so a mid-way failure rolls back and the BullMQ retry re-grants cleanly — never the old "claim consumed, codes lost" state. Producers stay best-effort to *enqueue* (a Redis blip must not 500 the triggering action).
- **Uploads go to Cloudflare R2 (local-disk fallback in dev).** `routes/uploads.ts` (5 MB cap, jpg/png/webp/heic) accepts *either* the user or admin cookie. With the four `R2_*` env vars set it stores to R2 and serves via the `GET /uploads/*` proxy (or straight from `R2_PUBLIC_URL` if set); without them it writes to `./uploads`. All storage logic stays in that module + the one `serveUploads` mount in `app.ts` — keep that boundary.
- **Webhooks.** `POST /webhooks/line` validates the LINE `x-line-signature` HMAC when `LINE_CHANNEL_SECRET` is set (falls back to the static `x-system-key` gate without it); event dispatch is still a logged stub. Maintenance tasks (idle-driver nudge; expire PENDING_PAYMENT jobs with no slip after `pending_payment_expire_days`, 0 = off) run as **BullMQ repeatable jobs** (`packages/services/src/support/queues/maintenance.ts`, patterns `CRON_NUDGE_SCHEDULE`/`CRON_EXPIRE_SCHEDULE`) — scheduling is coordinated in Redis so it's multi-instance safe. Their logic lives once in `packages/services/src/support/cron-tasks.ts`; the SYSTEM-only endpoints (`x-system-key`) `/webhooks/nudge-idle-drivers` and `/webhooks/expire-pending-payment` stay as thin wrappers for manual/external trigger. `notify()` writes the in-app `Notification` row synchronously (source of truth) and **enqueues** the LINE push onto the BullMQ notifications queue (`packages/services/src/support/queues/notifications.ts`, retry/backoff/rate-limit in the worker) — a failure of either channel must never break the triggering action.
- **Driver verification statuses:** REJECTED drivers auto-return to PENDING when they re-submit their profile; SUSPENDED is sticky — only an admin lifts it (suspension is punitive, not appealable by edit).
- **Cancellation fee is a snapshot, not a deduction.** `Job.cancellationFeeApplied` records the fee owed when a customer cancels after `freeCancelMinutes`; ops collects manually — there is no customer wallet. `Job.paymentRejectedCount` counts slip rejections; admins are notified at ≥3.
