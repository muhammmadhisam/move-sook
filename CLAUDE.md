...
- **Webhooks.** `POST /webhooks/line` validates the LINE `x-line-signature` HMAC when `LINE_CHANNEL_SECRET` is set (falls back to the static `x-system-key` gate without it); event dispatch is still a logged stub. Maintenance tasks (idle-driver nudge; expire PENDING_PAYMENT jobs with no slip after `pending_payment_expire_days`, 0 = off) run as **BullMQ repeatable jobs** (`apps/api/src/queues/maintenance.ts`, patterns `CRON_NUDGE_SCHEDULE`/`CRON_EXPIRE_SCHEDULE`) — scheduling is coordinated in Redis so it's multi-instance safe. Their logic lives once in `apps/api/src/lib/cron-tasks.ts`; the SYSTEM-only endpoints (`x-system-key`) `/webhooks/nudge-idle-drivers` and `/webhooks/expire-pending-payment` stay as thin wrappers for manual/external trigger. `notify()` writes the in-app `Notification` row synchronously (source of truth) and **enqueues** the LINE push onto the BullMQ notifications queue (`apps/api/src/queues/notifications.ts`, retry/backoff/rate-limit in the worker) — a failure of either channel must never break the triggering action.
