...
- **Webhooks.** `POST /webhooks/line` validates the LINE `x-line-signature` HMAC when `LINE_CHANNEL_SECRET` is set (falls back to the static `x-system-key` gate without it); event dispatch is still a logged stub. Cron-driven SYSTEM endpoints (`x-system-key`): `/webhooks/nudge-idle-drivers` and `/webhooks/expire-pending-payment` (auto-cancels PENDING_PAYMENT jobs with no slip after `pending_payment_expire_days`, 0 = off) — shell wrappers in `scripts/cron/`. `notify()` creates in-app `Notification` rows best-effort (a failure must never break the triggering action).
